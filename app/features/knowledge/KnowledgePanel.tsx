...
import type { Doc, PersonalKnowledge, Tab, OrgUnit } from "../shared-types";
import { PlusIcon } from "../../components/icons";
import { Pager } from "../Pager";
import { usePaged } from "../usePaged";
import KnowledgeDetailModal, { type KnowledgeSelection } from "./KnowledgeDetailModal";
...
export default function KnowledgePanel({ docs, personalKnowledge, knowledgeView, knowledgeQuery, knowledgeCategory, selectedKnowledgeIds, selectedPersonalKnowledgeIds, appRole, userEmail, orgUnits, setDocs, setNotice, setKnowledgeView, setKnowledgeQuery, setKnowledgeCategory, setSelectedKnowledgeIds, setSelectedPersonalKnowledgeIds, setShowUpload, setShowPersonalKnowledge, setInput, setTab, setAskMode, loadState, loadGovernance, loadPersonalKnowledge, deleteBatch, toggleSelectedId, setAllSelectedIds, downloadKnowledgeText }: KnowledgePanelProps) {
  ...
  const pagedDocs = usePaged(docs, 9);
  const pagedPersonal = usePaged(personalKnowledge, 9);
  ...
  return <section className="contentPanel">
    ...
    {!!deletableKnowledgeDocs.length&&<div className="bulkActionBar"><label><input type="checkbox" checked={deletableKnowledgeDocs.every(item=>selectedKnowledgeIds.includes(item.id))} onChange={event=>setAllSelectedIds(deletableKnowledgeDocs.map(item=>item.id),setSelectedKnowledgeIds,event.target.checked)}/>全选</label><button className="dangerButton" disabled={!selectedKnowledgeIds.length} onClick={deleteSelectedKnowledge}>删除选中（{selectedKnowledgeIds.length}）</button></div>}<div className="knowledgeGrid">{docs.length ? pagedDocs.pageItems.map(d => { const canDelete = appRole==="管理员" || d.createdBy===userEmail; return (<article key={d.id} className="knowledgeCard clickableCard" onClick={() => setSelection({ kind: "enterprise", doc: d })}><div className="cardTopRow"><span className="docIcon">{/\.(png|jpg|jpeg|webp)$/i.test(d.filename)?"▧":/\.(xlsx|csv)$/i.test(d.filename)?"▦":/\.(pdf)$/i.test(d.filename)?"▢":"▤"}</span>{canDelete && <label className="itemSelect" onClick={event => event.stopPropagation()}><input type="checkbox" checked={selectedKnowledgeIds.includes(d.id)} onChange={()=>toggleSelectedId(selectedKnowledgeIds,setSelectedKnowledgeIds,d.id)}/></label>}<div className="cardMenuWrap" onClick={event => event.stopPropagation()} onMouseLeave={() => setOpenDocMenu(null)}><button className="cardMenuTrigger" type="button" onClick={() => setOpenDocMenu(openDocMenu === d.id ? null : d.id)} aria-label="更多操作" title="更多操作">⋮</button>{openDocMenu === d.id && (<div className="cardMenu" role="menu"><button type="button" role="menuitem" onClick={() => { setSelection({ kind: "enterprise", doc: d }); setOpenDocMenu(null); }}>查看详情</button>{d.filename ? <a role="menuitem" href={`/api/state?download=${d.id}`}>下载文件</a> : <button type="button" role="menuitem" onClick={() => { downloadKnowledgeText(d.title, d.content); setOpenDocMenu(null); }}>下载文件</button>}<button type="button" role="menuitem" onClick={() => { syncKnowledge(d.id); setOpenDocMenu(null); }}>检查更新</button>{canDelete && <button type="button" role="menuitem" className="menuDanger" onClick={() => { deleteKnowledge(d); setOpenDocMenu(null); }}>删除</button>}</div>)}</div></div><h3>{d.title}</h3><div className="docMetaRow"><span className="docCategory">{d.category}</span><span className="docDot">·</span><span className="docVisibility">{d.visibility}</span><span className="docDot">·</span><span>V{d.version}</span><em className={d.status==="待解析"?"pending":""}>{d.status}</em></div><p className="docFilename">{d.filename||"在线文本"}<small>{d.sizeBytes?` · ${Math.max(1,Math.round(d.sizeBytes/1024))}KB`:" · 文本资料"}</small></p><small className="docUpdateMode">{d.updateMode}{d.updateSchedule?` · ${d.updateSchedule}`:""} · {d.createdBy}</small><div className="cardPrimaryAction"><button className="askAiButton" onClick={event=>{event.stopPropagation();askAboutDocument(d)}}>让AI调用</button></div></article>); }) : <div className="emptyState"><b>没有匹配的企业资料</b></div>}</div><Pager page={pagedDocs.page} pageSize={pagedDocs.pageSize} total={pagedDocs.total} onChange={pagedDocs.setPage} /></>:<><div className="sectionTitle"><div><h2>我的个人知识库</h2></div><button onClick={()=>setShowPersonalKnowledge(true)}><PlusIcon style={{ width: 12, height: 12 }} /> 新建个人知识</button></div>{!!personalKnowledge.length&&<div className="bulkActionBar"><label><input type="checkbox" checked={personalKnowledge.every(item=>selectedPersonalKnowledgeIds.includes(item.id))} onChange={event=>setAllSelectedIds(personalKnowledge.map(item=>item.id),setSelectedPersonalKnowledgeIds,event.target.checked)}/>全选</label><button className="dangerButton" disabled={!selectedPersonalKnowledgeIds.length} onClick={deleteSelectedPersonalKnowledge}>删除选中（{selectedPersonalKnowledgeIds.length}）</button></div>}<div className="knowledgeGrid">{personalKnowledge.length?pagedPersonal.pageItems.map(item => (<article key={item.id} className="knowledgeCard personalKnowledgeCard clickableCard" onClick={() => setSelection({ kind: "personal", item })}><div className="cardTopRow"><span className="docIcon personalIcon">私</span><label className="itemSelect" onClick={event => event.stopPropagation()}><input type="checkbox" checked={selectedPersonalKnowledgeIds.includes(item.id)} onChange={()=>toggleSelectedId(selectedPersonalKnowledgeIds,setSelectedPersonalKnowledgeIds,item.id)}/></label><div className="cardMenuWrap" onClick={event => event.stopPropagation()} onMouseLeave={() => setOpenPersonalMenu(null)}><button className="cardMenuTrigger" type="button" onClick={() => setOpenPersonalMenu(openPersonalMenu === item.id ? null : item.id)} aria-label="更多操作" title="更多操作">⋮</button>{openPersonalMenu === item.id && (<div className="cardMenu" role="menu"><button type="button" role="menuitem" onClick={() => { setSelection({ kind: "personal", item }); setOpenPersonalMenu(null); }}>查看详情</button><button type="button" role="menuitem" onClick={() => { downloadKnowledgeText(item.title, item.content); setOpenPersonalMenu(null); }}>下载文件</button><button type="button" role="menuitem" onClick={() => { syncPersonalKnowledge(item); setOpenPersonalMenu(null); }}>同步企业知识</button><button type="button" role="menuitem" className="menuDanger" onClick={() => { deletePersonalKnowledge(item); setOpenPersonalMenu(null); }}>删除</button></div>)}</div></div><h3>{item.title}</h3><div className="docMetaRow"><span className="docCategory">{item.sourceType}</span><em className={item.syncStatus==="已同步企业知识"?"":"pending"}>{item.syncStatus}</em></div><p className="docFilename">{item.content.replace(/[#>*_-]/g," ").slice(0,110)}{item.content.length>110?"…":""}</p><small className="docUpdateMode">{new Date(item.updatedAt).toLocaleString("zh-CN")}</small><div className="cardPrimaryAction"><button className="askAiButton" onClick={event=>{event.stopPropagation();askAboutPersonalKnowledge(item)}}>让AI调用</button></div></article>)) : <div className="emptyState"><b>还没有个人知识</b></div>}</div><Pager page={pagedPersonal.page} pageSize={pagedPersonal.pageSize} total={pagedPersonal.total} onChange={pagedPersonal.setPage} /></>}
    {selection && (selection.kind === "enterprise"
      ? <KnowledgeDetailModal selection={selection} canEdit={appRole === "管理员" || selection.doc.createdBy === userEmail} appRole={appRole} orgUnits={orgUnits} onClose={() => setSelection(null)} onSaved={() => Promise.all([loadState(), loadGovernance()]).then(() => undefined)} setNotice={setNotice} downloadKnowledgeText={downloadKnowledgeText} onAskAi={() => askAboutDocument(selection.doc)} onDelete={() => deleteKnowledge(selection.doc)} onSecondary={() => syncKnowledge(selection.doc.id)} />
      : <KnowledgeDetailModal selection={selection} canEdit={true} appRole={appRole} orgUnits={orgUnits} onClose={() => setSelection(null)} onSaved={() => Promise.all([loadPersonalKnowledge(), loadState()]).then(() => undefined)} setNotice={setNotice} downloadKnowledgeText={downloadKnowledgeText} onAskAi={() => askAboutPersonalKnowledge(selection.item)} onDelete={() => deletePersonalKnowledge(selection.item)} onSecondary={() => syncPersonalKnowledge(selection.item)} />)}
  </section>;
}
