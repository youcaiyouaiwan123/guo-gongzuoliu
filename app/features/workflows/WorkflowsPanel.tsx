...
export default function WorkflowsPanel({ workflows, workflowRuns, selectedWorkflowIds, runWorkflow, runningWorkflow, setRunningWorkflow, runInput, selectedRun, setModalType, setSelectedWorkflowIds, setRunWorkflow, setRunInput, setSelectedRun, chooseWorkflowTemplate, setAllSelectedIds, toggleSelectedId, deleteSelectedModules, runModule, deleteModule, loadModules, setNotice, loadState, loadGovernance, loadArtifacts }: WorkflowsPanelProps) {
  ...
  return <section className="contentPanel">
    ...
    <div className="workflowGrid">{workflows.length ? workflows.map(w=>{const nodes=parseWorkflowNodes(w);return <article key={w.id} className="workflowCard"><div className="workflowCardTop"><div><h3>{w.name}</h3><p>{w.triggerType} · {nodes.length} 个真实任务 · {new Set(nodes.filter(n=>n.parallelGroup).map(n=>n.parallelGroup)).size ? "含并行汇聚":"顺序执行"}</p></div><em>{w.status}</em></div><div className="workflowPath">{nodes.map((node,index)=><span key={node.id} className={node.parallelGroup?"parallelPathNode":""}><b>{node.parallelGroup?"并":index+1}</b>{node.name}{index<nodes.length-1&&<i>{node.parallelGroup&&nodes[index+1]?.parallelGroup===node.parallelGroup?<PlusIcon style={{ width: 10, height: 10 }} />:"→"}</i>}</span>)}</div><div className="workflowActions"><small>{w.lastRunAt?`上次运行：${new Date(w.lastRunAt).toLocaleString("zh-CN")}`:"尚未运行"}</small><div><button onClick={()=>runModule("workflow",w.id,w.name)}><PlayIcon style={{ width: 12, height: 12 }} /> 试运行</button><label className="itemSelect"><input type="checkbox" checked={selectedWorkflowIds.includes(w.id)} onChange={()=>toggleSelectedId(selectedWorkflowIds,setSelectedWorkflowIds,w.id)}/>选择</label><button className="dangerButton" onClick={()=>deleteModule("workflow",w.id,w.name)}>删除</button></div></div></article>}) : <div className="emptyState"><b>还没有工作流</b></div>}</div>
    {!!workflowRuns.length&&<><div className="sectionTitle subTitle"><div><h2>最近运行</h2></div><button className="outline" onClick={loadModules}>刷新</button></div><div className="runList">{workflowRuns.map(run=><button key={run.id} onClick={async()=>{const response=await fetch(`/api/modules?runId=${run.id}`);setSelectedRun(response.ok?await response.json():run)}}><span className={`runState ${run.status}`}>{run.status}</span><b>{run.workflowName}</b><small>#{run.id} · {run.actor} · {new Date(run.startedAt).toLocaleString("zh-CN")}</small><i>查看详情 →</i></button>)}</div></>}
