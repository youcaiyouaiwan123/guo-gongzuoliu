...
export default function Console({ userEmail, displayName }: { userEmail: string; displayName: string }) {
  ...
  const pagedConversations = usePaged(conversations, 12);
  ...
  const [continuous, setContinuous] = useState({ name: "", loopType: "目标制", reviewMode: "明确标准", goal: "", triggerType: "手动触发", reviewStandard: "", stopCondition: "", maxLoops: "3", finalAction: "提交管理员审批", failureAction: "通知负责人", scheduleTime: "09:00", watchSourceType: "free", watchSourceRef: "", triggerCondition: "", checkInterval: "10", dataSourceRef: "", knowledgeDocs: [] as string[] });
  ...
  // 每页条数由视口自适应计算：先量可用区，再按卡片尺寸算「列×行」铺满、不产生滚动条。
  // 初值仅占位，真正的值在 artifacts 标签挂载后由 ResizeObserver 效应写入。
  const [artifactsPageSize, setArtifactsPageSize] = useState(9);
  const artifactGridViewportRef = useRef<HTMLDivElement>(null);
  ...
  async function loadArtifacts(page = artifactsPage, owner = artifactOwner, pageSize = artifactsPageSize) {
  ...
  // 自适应每页条数：量出沉淀网格的可用区，按卡片固定尺寸算「列×行」，
  // 使每页正好铺满、既不留大片空白，也不触发横向/竖向滚动条。
  // 下面三个常量必须与 artifacts.css 保持一致：CARD_MIN_W=minmax 下限、GAP=grid gap、CARD_H=grid-auto-rows。
  useEffect(() => {
    if (tab !== "artifacts" || typeof window === "undefined") return;
    const el = artifactGridViewportRef.current;
    if (!el || typeof ResizeObserver === "undefined") return;
    const GAP = 12, CARD_MIN_W = 240, CARD_H = 236;
    let applied = 0;
    const recalc = () => {
      const w = el.clientWidth, h = el.clientHeight;
      if (w <= 0 || h <= 0) return;  // 移动端 height:auto 或未布局时跳过
      const cols = Math.max(1, Math.floor((w + GAP) / (CARD_MIN_W + GAP)));
      const rows = Math.max(1, Math.floor((h + GAP) / (CARD_H + GAP)));
      const size = Math.min(100, cols * rows);
      if (size !== applied) {
        applied = size;
        setArtifactsPageSize(size);
        loadArtifacts(1, artifactOwner, size);
      }
    };
...
${content}`,
  ...
  async function saveModule(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice(modalType==="source" ? (data.message || "采集任务已保存") : (data.message || (agentAction==="draft"?"智能体草稿已保存":"已保存并启用"))); setModalType(null); setEditingSource(null); setSourceDraft(null); await Promise.all([loadModules(), loadState()]);
    // 待审批的智能体尚未启用，不能直接试聊；等审批通过后再使用。
    if (modalType==="agent" && agentAction==="test" && data.id && !data.pendingApproval) {
  ...
  function editWorkflow(w: Workflow) {
    ...
    try {
      ...
      const knowledgeNode = Array.isArray(parsed) ? parsed.find(n => n.id === "loop-knowledge") : null;
      knowledgeDocs = Array.isArray(knowledgeNode?.knowledgeDocs) ? knowledgeNode!.knowledgeDocs : [];
    } catch { dataSourceRef = ""; }
    setContinuous({
      name: w.name,
  ...
  async function createContinuousTask(event: FormEvent) {
    ...
    // 接数据源：定时制/主动制创建时若选了数据源，就在"设定目标"之后、"执行任务"之前插一个
    // 数据采集节点，让到点/命中后真正执行时先拉到新鲜数据再交给 AI。
    // 主动制·指定数据源时默认复用所监测的那个源（watchSourceRef），也可被显式选择覆盖。
    ...
    const nodes = [
      ...
      ...(Number(execSourceId) > 0
        ...
        : []),
      ...(continuous.knowledgeDocs.length
        ? [{ id: "loop-knowledge", type: "knowledge", name: "知识检索", knowledgePick: "files", knowledgeDocs: continuous.knowledgeDocs }]
        : []),
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {tab === "chat" && <div className={`chatLayout ${insightCollapsed?"insightCollapsed":""}`}>
        <aside className="conversationPanel"><button className="newChatButton" onClick={newChat}><PlusIcon style={{ width: 14, height: 14 }} /> 新建独立任务</button>{!!conversations.length&&<div className="conversationBulk"><label><input type="checkbox" checked={conversations.every(item=>selectedConversationIds.includes(item.id))} onChange={event=>setAllSelectedIds(conversations.map(item=>item.id),setSelectedConversationIds,event.target.checked)}/>全选</label><button disabled={!selectedConversationIds.length} onClick={deleteSelectedChats}>删除选中</button></div>}<div className="conversationList">{pagedConversations.pageItems.map(item=><div key={item.id} className={conversationId===item.id?"active":""}><div className="conversationTitleRow"><label className="conversationCheck"><input type="checkbox" checked={selectedConversationIds.includes(item.id)} onChange={()=>toggleSelectedId(selectedConversationIds,setSelectedConversationIds,item.id)}/></label><button className="conversationTitle" onClick={()=>loadChats(item.id)}>{item.title}</button><button title="重命名" aria-label={`编辑${item.title}`} onClick={()=>renameChat(item)}>编辑</button><button title="删除" aria-label={`删除${item.title}`} onClick={()=>deleteChat(item.id)}>删除</button></div><button className="conversationMeta" onClick={()=>loadChats(item.id)}>{new Date(item.updatedAt).toLocaleDateString("zh-CN")} · {modelModeShortLabel(item.modelMode)}</button></div>)}</div><Pager page={pagedConversations.page} pageSize={pagedConversations.pageSize} total={pagedConversations.total} onChange={pagedConversations.setPage} />{!conversations.length && <div className="conversationEmpty"><div className="conversationEmptyIcon"><MessageIcon style={{ width: 32, height: 32 }} /></div></div>}</aside>
      ...
      {tab === "artifacts" && <section className="artifactsLayout pageFill">
        <aside className="artifactsSidebar">
          ...
          <main className="artifactsMain">
            ...
            <div className="artifactGridViewport" ref={artifactGridViewportRef}>{artifactsTotal ? <div className="artifactGrid">{visibleArtifacts.map(item => <article className="artifactCard" key={item.id}>
            ...
            </article>)}</div> : <div className="emptyState big">
              <b>还没有沉淀内容</b>
              <small>把对话整理为 Markdown 或 Skill，或在左侧拖入 .md / .skill / .txt 文件开始沉淀。</small>
            </div>}
            </div>
            {artifactsTotal > 0 && <Pager page={artifactsPage} pageSize={artifactsPageSize} total={artifactsTotal} alwaysShow onChange={next => loadArtifacts(next)} />}
          </main>
      </section>}
      ...
      {tab === "models" && <ModelsPanel modelStatus={modelStatus} selectedPresetModels={selectedPresetModels} setSelectedPresetModels={setSelectedPresetModels} presetApiKey={presetApiKey} setPresetApiKey={setPresetApiKey} addingPresetModels={addingPresetModels} setAddingPresetModels={setAddingPresetModels} showCustomModel={showCustomModel} setShowCustomModel={setShowCustomModel} editingModel={editingModel} setEditingModel={setEditingModel} testingModelId={testingModelId} setTestingModelId={setTestingModelId} selectedModelConnectionIds={selectedModelConnectionIds} setSelectedModelConnectionIds={setSelectedModelConnectionIds} modelMode={modelMode} setModelMode={setModelMode} setNotice={setNotice} setTab={setTab} loadModelStatus={loadModelStatus} deleteBatch={deleteBatch} setAllSelectedIds={setAllSelectedIds} toggleSelectedId={toggleSelectedId} />}
    ...
    {showApproval && <div className="modalBackdrop" onMouseDown={()=>setShowApproval(false)}><form className="modal" onSubmit={submitApproval} onMouseDown={e=>e.stopPropagation()}><div className="modalHead"><div><h2>发起企业审批</h2><p>从企业架构中指定具体审批账号，申请只会进入该账号的待办。</p></div><button type="button" onClick={()=>setShowApproval(false)}>×</button></div><label>审批类型<select name="requestType"><option>导出敏感数据</option><option>对外发送内容</option><option>创建智能体</option><option>运行自动化</option><option>其他事项</option></select></label>{approvalTargets.length ? <label>指定审批人<select name="approverEmail" required defaultValue={approvalTargets.find(item=>item.recommended)?.email||""}><option value="" disabled>请选择企业架构中的账号</option>{approvalTargets.map(item=><option key={item.email} value={item.email}>{item.recommended?"推荐 · ":""}{item.unitName} · {item.jobTitle} · {item.email}</option>)}</select></label> : <div className="emptyState"><b>暂无可指定的审批人</b><small>请先到「企业架构」添加在岗成员（你之外的账号），审批申请只能提交给企业架构中的人。</small></div>}<label>审批事项<input name="title" required placeholder="例如：导出本月重点客户清单"/></label><label>申请原因<textarea name="reason" required rows={7} placeholder="说明业务目的、数据范围、使用人和完成时间…"/></label><div className="modalActions"><button type="button" className="outline" onClick={()=>setShowApproval(false)}>取消</button><button type="submit" disabled={!approvalTargets.length}>提交给指定审批人</button></div></form></div>}
